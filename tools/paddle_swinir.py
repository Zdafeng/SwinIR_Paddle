...
class WindowAttention(nn.Layer):
    ...
    def __init__(self,
        ...
        self.relative_position_bias_table = self.create_parameter(
            shape=((2 * window_size[0] - 1) * (2 * window_size[1] - 1), num_heads),  # 2*Wh-1 * 2*Ww-1, nH
...
class SwinTransformerBlock(nn.Layer):
    ...
    def forward(self, x, x_size):
        ...
        if self.shift_size > 0 and self.input_resolution != x_size:
...
class PatchMerging(nn.Layer):
    ...
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm):
        ...
        self.norm = eval(norm_layer)(4 * dim)
...
class BasicLayer(nn.Layer):
    ...
    def extra_repr(self):
        return "dim={}, input_resolution={}, depth={}".format(self.dim, self.input_resolution, self.depth)
...
class RSTB(nn.Layer):
    ...
    def __init__(self,
        ...
        if resi_connection == '1conv':
            self.conv = nn.Conv2D(dim, dim, 3, 1, 1, bias_attr=True)
        elif resi_connection == '3conv':
            # to save parameters and memory
            self.conv = nn.Sequential(
                nn.Conv2D(dim, dim // 4, 3, 1, 1, bias_attr=True),
                nn.LeakyReLU(negative_slope=0.2),
                nn.Conv2D(dim // 4, dim // 4, 1, 1, 0, bias_attr=True),
                nn.LeakyReLU(negative_slope=0.2),
                nn.Conv2D(dim // 4, dim, 3, 1, 1, bias_attr=True))
...
class PatchUnEmbed(nn.Layer):
    ...
    def forward(self, x, x_size):
        x = x.transpose([0, 2, 1]).reshape([x.shape[0], self.embed_dim, x_size[0], x_size[1]])
        return x
    ...
    def __init__(self, scale, num_feat):
        ...
        if (scale & (scale - 1)) == 0:  # scale = 2^n
            for _ in range(int(math.log(scale, 2))):
                m.append(nn.Conv2D(num_feat, 4 * num_feat, 3, 1, 1, bias_attr=True))
                m.append(nn.PixelShuffle(2))
        elif scale == 3:
            m.append(nn.Conv2D(num_feat, 9 * num_feat, 3, 1, 1, bias_attr=True))
...
class UpsampleOneStep(nn.Sequential):
    ...
    def __init__(self, scale, num_feat, num_out_ch, input_resolution=None):
        ...
        self.input_resolution = input_resolution
        m = []
        m.append(nn.Conv2D(num_feat, (scale ** 2) * num_out_ch, 3, 1, 1, bias_attr=True))
...
class SwinIR(nn.Layer):
    r""" SwinIR
        A paddle impl of : `SwinIR: Image Restoration Using Swin Transformer`, based on Swin Transformer.
    Args:
    ...
    def __init__(self,
        ...
        self.conv_first = nn.Conv2D(num_in_ch, embed_dim, 3, 1, 1, bias_attr=True)
        ...
        self.norm = eval(norm_layer)(self.num_features)

        # build the last conv layer in deep feature extraction
        if resi_connection == '1conv':
            self.conv_after_body = nn.Conv2D(embed_dim, embed_dim, 3, 1, 1, bias_attr=True)
        elif resi_connection == '3conv':
            # to save parameters and memory
            self.conv_after_body = nn.Sequential(
                nn.Conv2D(embed_dim, embed_dim // 4, 3, 1, 1, bias_attr=True),
                nn.LeakyReLU(negative_slope=0.2),
                nn.Conv2D(embed_dim // 4, embed_dim // 4, 1, 1, 0, bias_attr=True),
                nn.LeakyReLU(negative_slope=0.2),
                nn.Conv2D(embed_dim // 4, embed_dim, 3, 1, 1, bias_attr=True))

        # ------------------------- 3, high quality image reconstruction ------------------------- #
        if self.upsampler == 'pixelshuffle':
            # for classical SR
            self.conv_before_upsample = nn.Sequential(
                nn.Conv2D(embed_dim, num_feat, 3, 1, 1, bias_attr=True), nn.LeakyReLU())
            self.upsample = Upsample(upscale, num_feat)
            self.conv_last = nn.Conv2D(num_feat, num_out_ch, 3, 1, 1, bias_attr=True)
            ...
            self.conv_before_upsample = nn.Sequential(
                nn.Conv2D(embed_dim, num_feat, 3, 1, 1, bias_attr=True), nn.LeakyReLU())
            self.conv_up1 = nn.Conv2D(num_feat, num_feat, 3, 1, 1, bias_attr=True)
            self.conv_up2 = nn.Conv2D(num_feat, num_feat, 3, 1, 1, bias_attr=True)
            self.conv_hr = nn.Conv2D(num_feat, num_feat, 3, 1, 1, bias_attr=True)
            self.conv_last = nn.Conv2D(num_feat, num_out_ch, 3, 1, 1, bias_attr=True)
            self.lrelu = nn.LeakyReLU(negative_slope=0.2)
        else:
            # for image denoising and JPEG compression artifact reduction
            self.conv_last = nn.Conv2D(embed_dim, num_out_ch, 3, 1, 1, bias_attr=True)
    ...
    def check_image_size(self, x):
        ...
        x = paddle.nn.functional.pad(x, (0, mod_pad_w, 0, mod_pad_h), 'reflect')
        return x
    ...
    def forward(self, x):
        ...
        self.mean = paddle.cast(self.mean, x.dtype)
