...
def _blocking_effect_factor(im):
    ...
    block_horizontal_positions = np.arange(7, im.shape[3] - 1, 8)
    block_vertical_positions = np.arange(7, im.shape[2] - 1, 8)
    ...
    nonblock_horizontal_positions = np.setdiff1d(np.arange(0, im.shape[3] - 1), block_horizontal_positions)
    nonblock_vertical_positions = np.setdiff1d(np.arange(0, im.shape[2] - 1), block_vertical_positions)
...
def calculate_psnrb(img1, img2, crop_border, input_order='HWC', test_y_channel=False):
    ...
    img1 = paddle.transpose(img1, perm=[2, 0, 1]).unsqueeze(0) / 255.
    img2 = paddle.to_tensor(img2)
    img2 = paddle.transpose(img2, perm=[2, 0, 1]).unsqueeze(0) / 255.
    ...
    for c in range(img1.shape[1]):
        ...
        mse = mse.reshape([mse.shape[0], -1]).mean(1)
