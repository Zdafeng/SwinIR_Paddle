...
def main():
    ...
    for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
        ...
        img_lq = paddle.to_tensor(img_lq)
        img_lq = paddle.cast(img_lq, paddle.float32).unsqueeze(0)
...
def load(file_name):
    with open(file_name, 'rb') as f:
        state_dicts = pickle.load(f) if six.PY2 else pickle.load(f, encoding='latin1')
    return state_dicts
...
def define_model(args):
    ...
    state_dicts = load(args.model_path)
    model.set_state_dict(state_dicts[param_key_g])
...
def test(img_lq, model, args, window_size):
    if args.tile is None:
        ...
        E = paddle.zeros((b, c, h * sf, w * sf))
        E = paddle.cast(E, img_lq.dtype)
